...
class ShipPreviewHost(ctk.CTkFrame):
    ...
    def _refresh_status(self) -> None:
        ...
        if self._hide_layers:
            text += f"  ·  {self._hide_layers} shells hidden; Reset visibility restores the hull"
    ...
    def _set_hide_layers(self, layers: int) -> None:
        was_hidden = self._hide_layers > 0
        self._hide_layers = max(0, min(8, int(layers)))
        if self._hide_layers and not was_hidden and self._on_toast:
            self._on_toast("Thin ships can disappear when shells are hidden. Use Reset visibility to restore them.", "info")
        try:
