...
class PreviewPanel(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._on_validate_se2 = on_validate_se2
    ...
    def _ensure_subgrids_widgets(self):
        ...
        container.columnconfigure(1, weight=5, minsize=360)
    ...
    def _build_analytics_tab(self):
        ...
        self.coverage_label = ctk.CTkLabel(self.tab_analytics, text="", wraplength=680,
                                        justify="left", text_color=TacticalTheme.ORANGE_PRIMARY)
        self.coverage_label.pack(fill="x", padx=12, pady=(6, 0))
    ...
    def _apply_pending_analytics(self):
        ...
        self.coverage_label.configure(text=analytics_result.coverage_text)
        self.metric_labels["Blocks"].configure(text=f"{analytics_result.block_count:,}")
        qualifier = "" if analytics_result.is_complete else "~"
        self.metric_labels["PCU"].configure(text=f"{qualifier}{analytics_result.pcu_total:,}")
        self.metric_labels["Mass"].configure(text=f"{qualifier}{analytics_result.mass_total:,.2f}")
    ...
    def _build_resource_tree_text(analytics_result, comparison: Optional[ConversionComparison]) -> str:
        lines: List[str] = [analytics_result.coverage_text, ""]
        lines.append("ORES (ESTIMATED)")
        for ore, qty in analytics_result.ore_totals.items():
            lines.append(f"  - {ore}: {qty:,.2f}")
        lines.append("")
        lines.append("INGOTS (ESTIMATED)")
    ...
    def _build_se2_tab(self):
        ...
        ctk.CTkLabel(
            header_frame,
            text="Space Engineers 2 native compatibility",
            font=TacticalTheme.FONT_LARGE,
            text_color=TacticalTheme.TEXT_WHITE,
        ).pack(side="left")
        ...
        self.se2_status_title = ctk.CTkLabel(
            score_details,
            text="Native validation required",
            font=TacticalTheme.FONT_LARGE,
            text_color=TacticalTheme.CYAN_PRIMARY,
            anchor="w",
        )
        ...
        self.se2_status_desc = ctk.CTkLabel(
            score_details,
            text="Check the blueprint against installed SE2 definitions before exporting. Supported armor only; unsupported blocks stop conversion.",
            font=TacticalTheme.FONT_SMALL,
            text_color=TacticalTheme.TEXT_CYAN,
            wraplength=520,
            justify="left",
        ...
        ctk.CTkLabel(
            ...
            text="Native format and definition checks",
            font=TacticalTheme.FONT_NORMAL,
            text_color=TacticalTheme.ORANGE_PRIMARY,
        ).pack(anchor="w", padx=12, pady=(8, 4))
        ...
        self._set_textbox_content(
            ...
            "Select a blueprint and run native validation.\n"
        )
        ctk.CTkButton(scroll_frame, text="Validate against installed SE2",
                      command=lambda: self._on_validate_se2() if self._on_validate_se2 else None).pack(fill="x", pady=8)
    ...
    def _apply_pending_se2(self):
        ...
        self.se2_score_label.configure(text="—", text_color=TacticalTheme.ORANGE_PRIMARY)
        self.se2_status_title.configure(text="Native validation required", text_color=TacticalTheme.ORANGE_PRIMARY)
        self.se2_status_desc.configure(text="Run validation against the installed SE2 catalog. DLC counts and scripts do not establish engine compatibility.")
        self._set_textbox_content(self.se2_audit_textbox,
            f"{info.display_name}\n{info.grid_size} grid · {info.block_count:,} blocks\n\n"
            "Native grid.json export supports standard armor cubes, slopes, corners and inverted corners.\n"
            "Other blocks, skins and mechanical subgrids stop conversion with explicit diagnostics.\n"
            "Validation has not run for this blueprint.")

    def show_native_se2_report(self, report):
        color = TacticalTheme.GREEN_PRIMARY if report.se2_migratable else TacticalTheme.ORANGE_PRIMARY
        self.se2_score_label.configure(text=f"{report.supported_blocks}/{report.total_blocks}", text_color=color)
        status = "Catalog checks passed" if report.se2_migratable else "Conversion blocked"
        if not report.catalog_validated:
            status = "Catalog validation unavailable"
        self.se2_status_title.configure(text=status, text_color=color)
        self.se2_status_desc.configure(text="Supported blocks / total blocks. A catalog check is separate from opening and testing the result in SE2.")
    ...
    def clear_se2_transition(self):
        ...
        self.se2_status_desc.configure(text="Select a blueprint, then validate against installed SE2 definitions.")
        self._set_textbox_content(self.se2_audit_textbox, "Select a blueprint and run native validation.\n")
