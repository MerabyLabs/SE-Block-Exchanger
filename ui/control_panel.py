...
class ControlPanel(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ctk.CTkLabel(category_frame, text="Only catalog-valid, same-type and same-footprint swaps are enabled. Unsupported pairs are omitted.",
                     font=TacticalTheme.FONT_SMALL, text_color=TacticalTheme.TEXT_GRAY,
                     wraplength=250, justify="left").pack(fill="x", padx=14, pady=(0, 10))
    ...
    def set_category_options(self, categories, enabled_categories):
        ...
        for group_title, ids in groups:
            ...
            for category_id in ids:
                ...
                var = ctk.BooleanVar(value=bool(category.pairs) and category.name.lower() in enabled_lookup)
                self._category_vars[category.name] = var
                pair_count = len(category.pairs)
                checkbox = ctk.CTkCheckBox(
                    self.category_checks_frame,
                    text=f"{category_label(category.name)}  ·  {pair_count}" if pair_count else f"{category_label(category.name)} · unsupported",
                    state="normal" if pair_count else "disabled",
                    variable=var,
                    font=TacticalTheme.FONT_SMALL,
                    text_color=TacticalTheme.TEXT_WHITE,
