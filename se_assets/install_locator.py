...
def resolve_install(
    ...
    *,
    ...
    """
    Prefer a saved path, then optionally auto-detect.

    `allow_detect=False` honors an explicit user Clear (and isolates tests
    from a live Steam tree). First launch with no saved path may detect.
    """
    if saved_path:
        try:
        ...
        if saved is not None and allow_detect:
