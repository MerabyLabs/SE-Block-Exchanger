...
def _as_offset_array(offsets: Sequence | np.ndarray | None, count: int) -> np.ndarray:
    if offsets is None:
...
def _build_instance_columns(
    ...
    style_map: Dict[Tuple[str, str], BlockStyle] = {}
...
def _collect_batches(
    ...
    *,
    explode: bool,
    lod: bool,
    offsets_peel: Sequence | np.ndarray,
    offsets_decks: Sequence | np.ndarray,
    offsets_radial: Sequence | np.ndarray,
...
def pending_mwm_patches(
    ...
    for i, block in enumerate(blocks):
        if cancel is not None and (i & 255) == 0 and cancel():
            return out
        definition = catalog.get(block.type_id, block.subtype)
        if definition is None or not _needs_mwm(definition):
            continue
        key = definition.key
...
def _slice_batch(batch: CpuBatch, keep: Sequence[int]) -> CpuBatch:
    ...
    if inspect is not None and inspect.size:
...
def _slice_offsets(primary: np.ndarray, fallback: np.ndarray, idx: np.ndarray) -> np.ndarray:
    src = primary if primary is not None and primary.size else fallback
    return src[idx]
